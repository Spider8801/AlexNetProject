...
accuracy_metric = tf.keras.metrics.CosineSimilarity()
...
# Accuracy metric.
def accuracy(accuracy_metric, y_pred, y_true):
   accuracy_metric.update_state(y_pred, y_true)
...
for step, (batch_x, batch_y) in enumerate(train_data.take(training_steps), 1):
    ...
    if step % display_step == 0:
        ...
        acc = accuracy(accuracy_metric, pred, batch_y)
        print('\n', "step: %i, Training loss: %f, Training accuracy: %f" % (step, loss, acc))
        ...
        validation_acc = accuracy(accuracy_metric, pred, y_test[randomlist,:])
        print("Validation loss: %f, Validation accuracy: %f" % (validation_loss, validation_acc))
        ...
        with train_summary_writer.as_default():
            ...
            tf.summary.scalar('Validation Accuracy', validation_acc, step=step)
